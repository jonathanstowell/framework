Feature: Keyed Repository
	In order to create an application I need to access data
	I want to be able to select data

Scenario: Insert Single Record
	Given I have created a person keyed repository
	And I have saved the following person using the keyed repository
		| FirstName | LastName |
		| Jonathan  | Stowell  |
	When I select all records using the keyed repository
...
Scenario: Insert Multiple Records
	Given I have created a person keyed repository
	And I have saved the following people using the keyed repository
		| FirstName | LastName |
		| Jonathan  | Stowell  |
		| Carl      | Saunders |
		| Tobin     | Saunders |
...
Scenario: Delete Multiple Records
	...
	And I have saved the following people using the keyed repository
		...
		| Tobin     | Saunders |
		| Kamen     | Staykov  |
	When I delete the following people using the keyed repository
		| FirstName | LastName |
		| Tobin     | Saunders |
		| Kamen     | Staykov  |
...
Scenario: Select a Record by ID
	Given I have created a person keyed repository
	And I have saved the following people using the keyed repository
		| FirstName | LastName |
		| Jonathan  | Stowell  |
		...
		| Tobin     | Saunders |
		| Kamen     | Staykov  |
	When I select a record with 2 as its ID
	Then  the single result should contain Carl as a first name
	And  the single result should contain Saunders as a surname
...
Scenario: Select All Records Paged
	...
	And I have saved the following people using the keyed repository
		...
		| Tobin     | Saunders |
		| Kamen     | Staykov  |
	When I select all records paged with a page size of 2 using the keyed repository
