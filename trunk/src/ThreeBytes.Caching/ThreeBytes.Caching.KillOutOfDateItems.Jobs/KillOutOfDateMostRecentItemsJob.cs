...
{
    public class KillOutOfDateMostRecentItemsJob : IJob
    {
        ...
        {
            logger.Info("Executing Kill Out of Date Most Recent Items Job");

            foreach (var configuration in cacheSettings.CacheItemConfigurations)
            {
                string key = cacheKeyGenerator.MostRecentPrimaryCacheKey(configuration.Value.ConfigurationType);
                ...
                foreach (var collectionItem in orderedCollectionsContainer.Value)
                {
                    if (collectionItem.Value.CacheUntil < DateTime.Now)
                    {
                        ...
                        logger.Info(string.Format("Removing Most Recent cache item: {0} > {1}", key, collectionItem.Key));
                    }
                }
            }

            logger.Info("Finished Kill Out of Date Most Recent Items Job");
        }
    }
}